...
export default function CategoryFilter({ active }: { active: NewsCategorySlug | null }) {
  const chip = (href: string, label: string, isActive: boolean) => (
    <Link
      key={href}
      href={href}
      className={`font-mono text-[12px] px-3 py-1.5 rounded-full border transition-colors outline-none focus-visible:ring-2 focus-visible:ring-foreground focus-visible:ring-offset-2 focus-visible:ring-offset-background ${
        isActive
